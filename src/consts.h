#pragma once
#include <QString>

const QString PROJECT_MODELS_DIR = "models";
const QString PROJECT_LEVELS_DIR = "levels";
const QString PROJECT_SOUNDS_DIR = "sound";
const QString PROJECT_LEVELS_META_FILE = "levels.json";
const QString PROJECT_THEME_FILE = "theme.json";
const QString DEFAULT_SCENE_LAYER_ID = "default";
// At this moment only English language is supported
const QString PROJECT_TRANSLATION_FILE = "text/en.json";
